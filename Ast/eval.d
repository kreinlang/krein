	...
	void import_module(Node data){
		...
		if (typeid(data.expr) == typeid(NullNode)){
			foreach(string i, u; data.strs){
				paths = config.check(IMPORTED_MODULES["environ"].attrs["path"], i, IMPORTED_MODULES);
			...
			foreach(string i, u; data.strs){
				...
				Module_path = config.check(IMPORTED_MODULES["environ"].attrs["path"], path, IMPORTED_MODULES);
