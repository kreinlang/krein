...
class Write: DataType{
	override DataType __call__(DataType[] params){
		string path = params[0].__str__;

		if (params.length > 1)
			std.file.write(path, params[1].__str__);
		else
			std.file.write(path, "");
...
class Writebin: DataType{
	override DataType __call__(DataType[] params){
		if (params.length){
			...
			if (params.length > 1)
				std.file.write(path, params[1].__chars__);
			else
				std.file.write(path, "");
		}
		return new None();
...
class Appendbin: DataType{
	override DataType __call__(DataType[] params){
		if (params.length){
			...
			if (params.length > 1)
				append(path, params[1].__chars__);
			else
				append(path, "");
		}
		return new None();
...
class Rename: DataType{
	...
	override string __str__() {return "rename (file method)"; }
}
...
class Remove: DataType{
	...
	override string __str__() {return "remove (file method)"; }
}
